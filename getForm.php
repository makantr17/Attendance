...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if ($_POST["email"] == "" ) {
        ...
        $email = trim($_POST["email"]);
            ...
            // Validate new password
        if ($_POST["course"] == "" ) {
            $course_err = "Please select your cohort";
        }
        else{
            ...
            if($_POST["month"] == "Choose the month" ){
                ...
                if ($_POST["From"] == "") {
                    ...
                    if ($_POST["To"] == "") {
                        ...
                        $script = '';
                        $query = 'SELECT '."$email".' info in '."$courseName".' course from '."$dateFrom".' To '."$dateTo".' in '."$month";
                        ...
                        foreach ($myarr as $nameDay) {
                            ...
                            for ($d=0; $d < sizeof($explodeValue) ; $d++) { 
                                    ...
                                    if ($dateFrom <= $divide[0] and $dateTo >= $divide[0] and $courseName ==$divide[2]) {
                                        if ($script == '') {
